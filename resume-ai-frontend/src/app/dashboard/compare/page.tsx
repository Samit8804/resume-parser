...
export default function ComparePage() {
  ...
  if (!data) return <div className="text-center py-12 text-gray-500">Failed to load comparison</div>

  const { comparison, skillUniverse } = data

  return (
    <div className="space-y-6">
      <Link href={`/dashboard/jobs/${searchParams.get("jobId")}`} className="text-sm text-blue-600 hover:underline inline-block">&larr; Back to job</Link>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-gray-200">
              <th className="text-left p-3 text-sm font-medium text-gray-500 w-40">Field</th>
              {comparison.map((c: any) => (<th key={c.id} className="p-3 text-left"><Link href={`/dashboard/candidates/${c.id}`} className="text-blue-600 hover:underline font-medium">{c.name}</Link></th>))}
            </tr>
          </thead>
          <tbody>
            {[
              { label: "Match Score", render: (c: any) => <span className={`text-lg font-bold ${c.matchScore >= 80 ? "text-green-600" : c.matchScore >= 60 ? "text-yellow-600" : "text-red-600"}`}>{Math.round(c.matchScore)}%</span> },
              ...
              { label: "Skills", render: (c: any) => <div><div className="flex flex-wrap gap-1">{c.skills.map((s: string) => (<Badge key={s} variant="outline">{s}</Badge>))}</div><span className="text-xs text-gray-400">{c.skillCount} total</span></div> },
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead><tr className="border-b border-gray-200">
              <th className="text-left p-2 text-sm font-medium text-gray-500">Skill</th>
            ...
            <tbody>
              {skillUniverse.map((skill: string) => (
                <tr key={skill} className="border-b border-gray-100">
                  ...
                  {comparison.map((c: any) => (<td key={c.id} className="p-2 text-center">{c.skills.includes(skill) ? <span className="text-green-600 font-bold">&#10003;</span> : <span className="text-red-300">&mdash;</span>}</td>))}
                </tr>
              ))}
            </tbody>
          </table>
